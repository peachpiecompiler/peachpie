    ...
    internal class PEParameterSymbol : ParameterSymbol
    {
        ...
        public override bool HasNotNull
        {
            get
            {
                ...
                {
                    // [NotNull] generated by Peachpie
                    // TODO: Consider removing when emitting nullability in C# 8.0 style
                    if (!_packedFlags.TryGetWellKnownAttribute(WellKnownAttributeFlags.NotNull, out bool isNotNull))
                    {
                        isNotNull = _packedFlags.SetWellKnownAttribute(WellKnownAttributeFlags.NotNull, AttributeHelpers.HasNotNullAttribute(Handle, (PEModuleSymbol)ContainingModule));
                    }

                    // C# 8.0 Nullability check
                    value = isNotNull ? true : AttributeHelpers.IsNotNullable(this, Handle, (PEModuleSymbol)ContainingModule);
